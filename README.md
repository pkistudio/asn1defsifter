...
The `Corpora` menu opens a session-scoped corpus manager for the ASN.1 definitions used by candidate resolution. The built-in `PkiComponents` corpus is active by default. Additional corpora can be added from a local file or clipboard ASN.1 text, edited in place, or removed from the active list. Corpus names must be unique and are used as the corpus/module names shown in candidate results. `Revert` restores the initial built-in corpus state and discards session additions or edits. When input is already loaded, corpus changes refresh the candidate report automatically.
